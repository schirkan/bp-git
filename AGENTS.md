...
- **Hook-Status (Stand 2026-09-01):** `PreReceiveHandler` / `PostReceiveHandler` / `PostCheckoutHandler` sind produktiv verdrahtet (Hybrid-Ansatz per SPEC-pre-receive-wiring.md §1.3, Implementation in `src/BPGit.Server/GitHttp/PushOrchestrator.cs`). `PushOrchestrator` sitzt zwischen HTTP-Request-Body und `git receive-pack --stateless-rpc`, parst Ref-Update-Pkt-Lines (`PktReader.cs` + `RefUpdateParser.cs`), ruft nach ref-apply `PreReceiveHandler.HandleAsync` pro Ref-Update (außer Delete) und `PostReceiveHandler.HandleAsync` für Worktree-Materialization. **Side-effect post-apply per Spec §9:** `PreReceive` läuft NACH ref-update, **kann Push nicht ablehnen** bei BP-DB-Sync-Fehler. `PostCheckoutHandler` läuft nach `git-upload-pack` für Worktree-Refresh. Library-Recherche + Empfehlung in SPEC-pre-receive-wiring.md §1 dokumentiert (Commit `0fc26f3`).
